...
public class MiServlet extends HttpServlet {
			...
			try {
				...
				for (FileItem item : items) {
					if (item.isFormField()) {
						...
						switch (fieldName) {
						case "nombre":
							if (fieldValue.matches(".*\\d.*")) { // Verifica si hay números en el nombre
	...
	private boolean validarEdad(String fechaNacimiento) {
		...
		sdf.setLenient(false); // Evita aceptar fechas inválidas como "2024-02-30"
		...
		try {
			...
			long edadMs = hoy.getTime() - fecha.getTime();
			int edad = (int) (edadMs / (1000L * 60 * 60 * 24 * 365));

			return edad >= 18 && edad <= 60; // Devuelve true si está en el rango válido
